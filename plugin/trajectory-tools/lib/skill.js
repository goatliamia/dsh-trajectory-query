...
export const SKILL_BODY = `# trajectory-query(查询纪律 v4)
...
- \`trajectory_cost\` — 成本:逐请求(或 \`groupBy:'turn'\`)的 input / cacheRead / cacheWrite / output / reasoning / total 与 **cache 效率** = cacheRead/(cacheRead+input);适配器没上报 usage 的请求记 \`unknown\`

默认行为:

- 不给 session 时,\`find\` / \`index\` 扫描 **当前会话 + 所有 live 会话 + 最近若干已持久化会话**;要查很久以前的已持久化会话,显式传 session。
- 默认过滤注入样板(\`<system-reminder>\` 的 workspace 指令)与 \`trajectory_*\` 自身的调用/结果,避免"查什么就命中自己";要看原始结果时关掉 \`excludeInjected\` / \`excludeSelf\`。\`cost\` 不套这两个过滤:记账要记全量。
- 匹配默认归一化(\`normalize:true\`):连续反斜杠折叠、中英文引号互认 —— 写单反斜杠的路径也能查到日志里转义过的双反斜杠。

成本纪律:

- 数字是**确定性事实**,工具只给数;"这次花得值不值"是你的解释,不要混进数字里。
- 读成本时看三个量:\`input\`(本次真正新增的上下文)、\`cacheRead\`(复用掉的前缀)、\`cacheEfficiency\`(掉到 1.0 以下就说明前缀被改写或换了序列)。
...
cost(session=abc, groupBy="turn")       →  每轮 token 与 cache 效率
\`\`\`
`;
